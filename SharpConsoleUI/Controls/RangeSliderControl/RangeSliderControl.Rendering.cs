...
namespace SharpConsoleUI.Controls
{
	public partial class RangeSliderControl
	{
			...
			int startX, int y, Color defaultFg, Color bgColor)
		{
			...
			if (_showValueLabel)
				availableWidth -= UnicodeWidth.GetStringWidth(FormatValue(_maxValue)) * 2 + 1 + ControlDefaults.SliderLabelSpacing;
		...
		private Color ResolveThumbColor(ActiveThumb thumb)
		{
			...
			if (HasFocus && _activeThumb == thumb) return FocusedThumbColor ?? Color.Yellow;
