...
namespace SharpConsoleUI.Controls
{
	...
	/// </summary>
	public partial class ScrollablePanelControl : BaseControl, IInteractiveControl, IFocusableControl, IMouseAwareControl, IContainer, IContainerControl, IControlHost, IScrollableContainer, IFocusScope, ILogicalCursorProvider, IColorRoleableControl
	{
		...
		internal LayoutRect ResolveContentMetrics(LayoutRect outerBounds, bool clampOffsets = true)
		{
			...
			if (_metricsCacheValid && _metricsCacheBounds.Equals(outerBounds))
			{
				...
				if (clampVertical)
				{
					...
					if (_verticalScrollOffset > cachedMaxScrollOffset)
						SetVerticalScrollOffset(cachedMaxScrollOffset, "metrics-clamp-cached");
				}
				if (clampHorizontal)
				{
			...
			if (clampVertical)
			{
				...
				if (_verticalScrollOffset > maxScrollOffset)
					SetVerticalScrollOffset(maxScrollOffset, "metrics-clamp-max");
			}
			if (clampHorizontal)
			{
				if (_horizontalScrollOffset > MaxHorizontalScrollOffset)
