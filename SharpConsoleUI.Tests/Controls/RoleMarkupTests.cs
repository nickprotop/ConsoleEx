...
public class RoleMarkupTests
{
	...
	{
		var ctrl = new MarkupControl(new List<string> { "Hello" }) { Role = ControlRole.Danger };
		var buffer = Paint(ctrl);
		var fg = ForegroundOf(buffer, 'H');
		Assert.NotNull(fg);
		Assert.NotEqual(new Color(255, 255, 255), fg!.Value);

		var expected = SharpConsoleUI.Helpers.ColorResolver.RoleForeground(ControlRole.Danger, ctrl.Container, ctrl.Outline);
	...
	public void InlineColorTagOverridesRole()
	{
		...
		var ctrl = new MarkupControl(new List<string> { "[red]X[/]" }) { Role = ControlRole.Success };
		var buffer = Paint(ctrl);
		...
		var roleFg = SharpConsoleUI.Helpers.ColorResolver.RoleForeground(ControlRole.Success, ctrl.Container, ctrl.Outline);
	...
	public void Builder_RoundTripsRoleAndOutline()
	{
		var ctrl = new MarkupBuilder()
			...
			.Outline()
			.Build();
		Assert.Equal(ControlRole.Success, ctrl.Role);
