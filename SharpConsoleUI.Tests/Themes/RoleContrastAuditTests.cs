...
/// <summary>
/// Safety net: every <see cref="ControlRole"/> resolved against every catalog theme must produce
...
/// </summary>
public class RoleContrastAuditTests
{
	private static readonly ControlRole[] Roles =
	{
		ControlRole.Primary, ControlRole.Secondary, ControlRole.Tertiary,
		ControlRole.Info, ControlRole.Success, ControlRole.Warning, ControlRole.Danger
	};
	...
	public void Role_Solid_Normal_IsReadable(string themeName, ControlRole role)
	{
		var reg = new ThemeRegistryStateService();
		var theme = reg.GetTheme(themeName)!;
		var rc = RoleResolver.Resolve(role, theme, outline: false, state: RoleState.Normal);
	...
	[Theory]
	[MemberData(nameof(Cases))]
	public void Role_Outline_Normal_IsReadable(string themeName, ControlRole role)
	{
		var reg = new ThemeRegistryStateService();
		var theme = reg.GetTheme(themeName)!;
		var rc = RoleResolver.Resolve(role, theme, outline: true, state: RoleState.Normal);
		var surfaceBg = theme.WindowBackgroundColor;
